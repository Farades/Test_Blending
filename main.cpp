...
#define TEX_IMAGE_NAME "D:/workspace/Test_Blending/Glass.bmp"

BOOL  light = true;      // Свет ВКЛ / ВЫКЛ
BOOL  blend = true;      // Смешивание НЕТ/ДА?
BOOL bp = true;
...
void KeyboardEvent(unsigned char key, int x, int y)
{
    switch(key)
    {
        ...
        case 'd' :
            ...
            break;
        case 'f' :
            filter++;           // значение filter увеличивается на один
...
void display()
{
    ...
    glBindTexture(GL_TEXTURE_2D, texture[filter%3]);    // Выбор текстуры основываясь на filter
